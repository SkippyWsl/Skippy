...
void CameraDevice::OnWarning(CrInt32u warning)
{
    ...
    tout << std::endl << "Warning: 0x" << std::hex << warning << std::dec << ". " << m_info->GetModel() << "(" << id.data() << ")\n";
}
...
void CameraDevice::OnError(CrInt32u error)
{
    ...
    if (error == SDK::CrError_Connect_TimeOut) {
        tout << "Device Disconnect Timeout. " << m_info->GetModel() << "(" << id.data() << ")\nPlease input '0' after Connect camera\n";
        return;
    }
    tout << std::endl << "Error: 0x" << std::hex << error << std::dec << ". " << m_info->GetModel() << "(" << id.data() << ")\n";
}
    ...
    if (prop_list && nprop > 0) {
        ...
        for (std::int32_t i = 0; i < nprop; ++i) {
            ...
            case SDK::CrDevicePropertyCode::CrDeviceProperty_FocusArea:
                ...
                if (nval != m_prop.focus_area.possible.size()) {
