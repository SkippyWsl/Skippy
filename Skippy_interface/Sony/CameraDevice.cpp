...
//    m_spontaneous_disconnection = false;
    ...
    auto connect_status = SDK::Connect(m_info, this, &m_device_handle, openMode, reconnect);
...
void CameraDevice::s1_shooting(int seconds) const
{
    //text input;
...
bool CameraDevice::af_shutter() const
{
    //while (block == true) {
    //    continue;
    //}
    ...
    std::int32_t nprop = 0;
    CrInt32u focusprop = SDK::CrDevicePropertyCode::CrDeviceProperty_FocusIndication;
    SDK::CrDeviceProperty* prop_list = nullptr;
    SDK::CrError status = SDK::CrError_Generic;
    try
    {
        for (int i = 0; i < 45; i++) {
            status = SDK::GetSelectDeviceProperties(m_device_handle, 1, &focusprop, &prop_list, &nprop);
            if (prop_list[0].GetCurrentValue() == 0x00000102) {
                SDK::SendCommand(m_device_handle, SDK::CrCommandId::CrCommandId_Release, SDK::CrCommandParam::CrCommandParam_Down);
                std::this_thread::sleep_for(35ms);
                SDK::SendCommand(m_device_handle, SDK::CrCommandId::CrCommandId_Release, SDK::CrCommandParam::CrCommandParam_Up);
                std::this_thread::sleep_for(1s);
...
{
    wchar_t  name{ L'Image' };
    text path = fs::current_path().native();
    tout << path.data() << '\n';

    auto save_status = SDK::SetSaveInfo(m_device_handle, const_cast<text_char*>(path.data()), &name , ImageSaveAutoStartNo);
...
long long int* CameraDevice::preview()
{
    if (block == true) {
        return nullptr;
    }
        ...
        else
        {
            ...
            {
                if (0 < image_data->GetSize() && bufflock == false)
                {
                    int counter = 0;
                    char* data = (char*)image_data->GetImageData();
                    while (counter < 307200) {
                        picturedata[counter] = data[counter];
                        counter++;
                    }
                    ...
                    //tout << "good\n";
                    block = false;
                    long long int* addr = reinterpret_cast<long long int*>(picturedata);
                    bufflock = true;
                    return addr;
                }
                else
...
bool CameraDevice::bufferunlock()
{
    bufflock = false;
    return true;
}
int CameraDevice::buffersize()
{
    size = sizeof(picturedata);
    return size;
}
void CameraDevice::set_path(CrChar* path, CrChar* prefix, CrInt32 no)
{
    auto save_status = SDK::SetSaveInfo(m_device_handle, path, prefix, no);
    if (CR_FAILED(save_status)) {
        //tout << "Failed to set save path.\n";
    }

}
int CameraDevice::battleft()
{
    std::int32_t nprop = 0;
    CrInt32u battprop = SDK::CrDevicePropertyCode::CrDeviceProperty_BatteryRemain;
    SDK::CrDeviceProperty* prop_list = nullptr;
    SDK::CrError status = SDK::CrError_Generic;
    //CrInt64u raw = 0;
    int raw = 0;
    //status = SDK::GetDeviceProperties(m_device_handle, &prop_list, &nprop);
       ...
       // Get difference
       // CrError GetSelectDeviceProperties(CrDeviceHandle deviceHandle, CrInt32u numOfCodes, CrInt32u* codes, CrDeviceProperty** properties, CrInt32* numOfProperties);
    status = SDK::GetSelectDeviceProperties(m_device_handle, 1, &battprop, &prop_list, &nprop);

    //if (CR_FAILED(status)) {
    //    return 0;
    //}

    if (CR_FAILED(status)or(nprop == 0)) {
        return 0;
    }
