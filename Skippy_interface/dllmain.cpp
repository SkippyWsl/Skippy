...
extern "C" Skippy_interface_API int battleft();
...
Skippy_interface_API int initialize()
{
    ...
    camera_list->Release();
    auto connect_status = camera->connect(Sony::CrSdkControlMode_Remote, Sony::CrReconnecting_ON);
    if (!connect_status) {
        Sony::Release();
        return 1;
    }
...
Skippy_interface_API bool focus(int seconds)
{
    ...
    return true; //TODO return false if no focus
}
Skippy_interface_API bool picture()
{
    //camera->af_shutter();
    return camera->af_shutter(); // TODO can return true or false /nofocus
}
...
Skippy_interface_API int battleft()
{
    return camera->battleft();
}