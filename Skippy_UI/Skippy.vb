...
Public Class Skippy
    ...
    Public camwait As Integer = 600
    Public finmov As Boolean
    'Public stopW As New Stopwatch
    Public battery As Integer = 0
    Public batterywarning As Boolean = False
    ...
    Private Sub Skippy_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Else
            ...
            For Each sp As String In Portnames 'workaround win load all serial drivers
                Try
                    Serial.PortName = sp
                    Serial.Open()
                    Serial.Close()
            ...
            For Each sp As String In Portnames
                Try
                    ...
                    'Thread.Sleep(1500)
                    stopW.Restart()
                    While stopW.ElapsedMilliseconds < 200
    ...
    Private Sub Serial_DataReceived(sender As Object, e As SerialDataReceivedEventArgs) Handles Serial.DataReceived
        ...
        If readBuffer.Contains("Arduino!") Then
            connected = True
            'MessageBox.Show("time:" & stopW.ElapsedMilliseconds)
            Scom(led + "125")
    ...
    Private Sub btn_picture_Click(sender As Object, e As EventArgs) Handles btn_picture.Click
        If loaded = 0 Then
            ...
            If Not Cam.picture() Then
                MessageBox.Show("No focus or Error taking image")
            End If
        Else
            MessageBox.Show("SDK not loaded or no camera conected")
        End If
    ...
    Private Sub captures()
        ...
        While mm < Int(tb_range.Text)
            ...
            Scom(stepwide)
            mm += stepwide
            If Advanced.Ctimer.Checked = False Then
                While finmov = False
    ...
    Private Sub slider_light_MouseUp(sender As Object, e As MouseEventArgs) Handles slider_light.MouseUp
        Scom(led & slider_light.Value)
    End Sub

    Private Sub updatebattery()
        If loaded = 0 Then
            Dim currentbat = Cam.battleft
            If currentbat = 5 And batterywarning = False Then
                MessageBox.Show("Battery has less than 5% charge left please recharge")
                batterywarning = True
            End If
            battery = currentbat
            battlabel.Text = ("Battery left:" + battery.ToString + "%")
        Else
            battlabel.Text = ("Battery left: N/A")
        End If
    End Sub

    Private Sub updatebatt_Tick(sender As Object, e As EventArgs) Handles updatebatt.Tick
        updatebattery()
    End Sub
End Class
